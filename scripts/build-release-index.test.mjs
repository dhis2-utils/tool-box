    ...
    assets: [
        ...
        },
        {
            ...
            browser_download_url:
                'https://github.com/dhis2/tool-box/releases/download/v0.1.5/tool-box.zip',
        },
    ],
}
...
test('stripV removes exactly one leading v, case-insensitively', () => {
...
test('fetchLatestRelease returns null on 404 and sends auth header when a token is given', async () => {
    ...
    assert.equal(
        await fetchLatestRelease('dhis2/none', { fetchImpl, token: 't' }),
        null
    )
    const found = await fetchLatestRelease('dhis2/tool-box', {
        fetchImpl,
        token: 'secret',
    })
...
test('fetchLatestRelease omits the auth header without a token', async () => {
    const fetchImpl = mockFetch({ 'dhis2/tool-box': release })
    await fetchLatestRelease('dhis2/tool-box', { fetchImpl, token: null })
...
test('fetchLatestRelease throws on any other error status', async () => {
    ...
    await assert.rejects(
        () => fetchLatestRelease('dhis2/broken', { fetchImpl, token: null }),
...
test('buildIndex keeps tools.json order and stamps generated_at', async () => {
    ...
    const index = await buildIndex(tools, { fetchImpl, token: null, now })
    assert.equal(index.generated_at, '2026-09-22T03:00:12.000Z')
    assert.deepEqual(
        index.tools.map((t) => t.repo),
        ['dhis2/none', 'dhis2/tool-box']
    )
    assert.equal(index.tools[0].version, null)
    assert.equal(index.tools[1].version, '0.1.5')
})

test('buildIndex aborts the whole run when a later tool fails, even after an earlier success', async () => {
    const tools = [
        { repo: 'dhis2/tool-box', name: 'DHIS2 Admin Toolbox' },
        { repo: 'dhis2/broken', name: 'Broken' },
    ]
    const fetchImpl = mockFetch({
        'dhis2/tool-box': release,
        'dhis2/broken': { status: 500 },
    })
    await assert.rejects(
        () => buildIndex(tools, { fetchImpl, token: null }),
        /GitHub API 500 for dhis2\/broken/
    )
})
